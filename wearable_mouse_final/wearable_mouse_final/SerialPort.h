    ...
    BYTE Parity, BYTE StopBits)
{
    ...
    {
        MessageBox(NULL, L"GetCommState Error", L"Error", MB_OK + MB_ICONERROR);
        CloseHandle(m_hComm);
        return false;
    }
    ...
    if (m_bPortReady == 0)
    {
        MessageBox(NULL, L"SetCommState Error", L"Error", MB_OK + MB_ICONERROR);
        CloseHandle(m_hComm);
        return false;
    }
    ...
    DWORD WriteTotalTimeoutMultiplier, DWORD WriteTotalTimeoutConstant)
{
    ...
    if (m_bPortReady == 0)
    {
        MessageBox(NULL, L"StCommTimeouts function failed", L"Com Port Error", MB_OK + MB_ICONERROR);
        CloseHandle(m_hComm);
        return false;
    }
